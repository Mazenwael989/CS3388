#Created by Mazen Baioumy
#Purpose: Complete a ray tracing program that displays spheres and computes shadows generated by objects.....
from math import *
import numpy as np
...
class cameraMatrix:
    ...
    def minimumIntersection(self,direction,objectList):
        ...
        # function to be used by sort to sort
        def takeSec(elem):
            return elem[1]
        ...
        for k in objectList:
            ...
            inverseofM = (k.getT()).inverse()
            # transform ray with M−1
            Ta = inverseofM*self.__E
            Tb = inverseofM*direction
            T0 = k.intersection(Ta, Tb)
            #if T0 doesnt equal 1 then add tuple to intersection list
            if ( T0 != -1.0):
                intersecList.append((m, T0))
            m+=1
        #sort intersection list in increasing order
        intersecList.sort(key=takeSec)
...
class shader:
    ...
    def __shadowed(self,object,I,S,objectList):
        # M = matrix B associated with object
        M = object.getB()
        #This operation detaches the intersection point from its surface and changes it to world coordinates
        I = M*(I + S.scalarMultiply(0.001))
        #Computing this transforms S into world coordinates
        S = M * S
        #Create for loop for object list
        for object in objectList:
        #inverse of matrix B associated with object
            inverseM = (object.getB()).inverse()
            #Transform the intersection point into the
            In = inverseM*I
            #transform the vector
            Sn = inverseM*S
            #if it doesnt equal 1 then their is intersection with another object
            if(object.intersection(In,Sn) != -1.0):
                return True
        return False
    ...
    def __init__(self,intersection,direction,camera,objectList,light):
        ...
        object = objectList[k]# declare object
        invM = (object.getT()).inverse()#inverse of matrix associated with object
        Ts = invM * light.getPosition()#Transformed light
        Te = invM * camera.getE()#E is position of cmera
        Td = invM * direction
        i = Te+Td.scalarMultiply(t0)#Calculate the intersection point
        s = (Ts - i).normalize()#Calculate the vector from the intersection point to light source position
        n = object.normalVector(i)#Calculate the normal vector at intersection point
        r = s.scalarMultiply(-1)+n.scalarMultiply(s.scalarMultiply(2).dotProduct(n))#Calculate the Sreflection vector
        v = (Te - i).normalize()#Calculate the vector to center of projection.
        x = max(n.dotProduct(s), 0)
        z = max(r.dotProduct(v),0)
        reflectance = object.getReflectance()#Calculate reflectance
        color = object.getColor()#Get the Color
        Lightintensity = light.getIntensity()#Get the Light intensity



        # if statement to decide if the intersection point is not shadowed by any other objects
        if (not(self.__shadowed(object, i, s, objectList))):
            # compute f first
            f = reflectance[0]+reflectance[1]*x + reflectance[2]*(z**reflectance[3])
        else: 
            # compute f again
            f = reflectance[0]
        # compute tuple
        self.__color = (int(round(color[0]*Lightintensity[0]*f)), int(round(color[1]*Lightintensity[1]*f)), int(round(color[2]*Lightintensity[2]*f)))
    def getShade(self):
        return self.__color
